...
bool try_key1()
{
    ...
    {    
      ...
      Serial.print(F("Authenticating using key A..."));Serial.println(k);
      Serial.print(key.keyByte[0], HEX);Serial.print(" ");
      Serial.print(key.keyByte[1], HEX);Serial.print(" ");
      Serial.print(key.keyByte[2], HEX);Serial.print(" ");
      Serial.print(key.keyByte[3], HEX);Serial.print(" ");
      Serial.print(key.keyByte[4], HEX);Serial.print(" ");
      Serial.print(key.keyByte[5], HEX);Serial.println(" ");
      status = mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, block, &key, &(mfrc522.uid));
      if (status != MFRC522::STATUS_OK) 
        {
          Serial.print(F("PCD_Authenticate() error: "));
          Serial.println(mfrc522.GetStatusCodeName(status));
          continue;
        }
...
//    }
    ...
    for(int j = 0; j < 16; j++)
    {
      Serial.print(waarde[block][j] < 0x10 ? " 0" : " ");
      Serial.print(waarde[block][j], HEX);
    }
    Serial.print("\n");
  }
...
//        // Copy the known key into the MIFARE_Key structure
//        for (byte i = 0; i < MFRC522::MF_KEY_SIZE; i++) {
//            key1.keyByte[i] = knownKeys[NR_KNOWN_KEYS-1][i];
//        }
////    }
//#endif    
    for (byte i = 0; i < 6; i++) 
  ...
  //for(int i = 4; i <= 63; i++)
  for(int i = 1; i <= 63; i++)  // including sector 0
  { //De blocken 4 tot 62 kopieren, behalve al deze onderstaande blocken (omdat deze de authenticatie blokken zijn)
    //Copy the blocks 4 to 62, except for all these blocks below (because these are the authentication blocks)
    ...
    block = i;
    Serial.print("-- write BLOCK ");Serial.println(block);
...
//          key.keyByte[i] = knownKeys[k][i];
//      }
//      Serial.print(F("Authenticating using key "));Serial.println(k);
//      Serial.print(key.keyByte[0], HEX);Serial.print(" ");
//      Serial.print(key.keyByte[1], HEX);Serial.print(" ");
//      Serial.print(key.keyByte[2], HEX);Serial.print(" ");
//      Serial.print(key.keyByte[3], HEX);Serial.print(" ");
